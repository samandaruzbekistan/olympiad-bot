...
<div class="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    <form action="{{ route('admin.olympiad-types.update', $olympiadType) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
            <label for="name" class="block text-base font-medium text-slate-700">Nomi</label>
            <input type="text" name="name" id="name" value="{{ old('name', $olympiadType->name) }}" required
